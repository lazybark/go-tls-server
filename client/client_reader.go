...
func (c *Client) reader() {
	for {
		...
		bytes, bytesCount, err := c.conn.ReadWithContext(c.conf.BufferSize, c.conf.MaxMessageSize, c.conf.MessageTerminator)
		if err != nil {
		...
		if bytesCount > 0 {
			c.messageChan <- conn.NewMessage(c.conn, bytesCount, bytes)
		}
	}
}
